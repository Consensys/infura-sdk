...
export class ExternallyOwnedAccount {
  ...
  async _mint(publicAddress, NFTImage) {
    if (this._contractAddress === null)
      throw new Error('[Contract.mint] You have to deploy the contract!');
  ...
  createSmartContract(name, symbol) {
    ...
    return {
      ...
      getOwnerOf: tokenId => this._getOwnerOf(tokenId),
    };
  }
  ...
  /*
   * Private method!
   * Get owner's public address by TokenID
   * @param  {number} tokenId
   * @return {string} public address
   */
  async _getOwnerOf(tokenId) {
    if (this._contractAddress === null)
      throw new Error('[Contract.getOwnerOf] Please deploy the contract.');

    if (!Number.isInteger(tokenId))
      throw new Error('[Contract.getOwnerOf] Param tokenId needs to be a number.');
