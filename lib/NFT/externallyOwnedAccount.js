...
export class ExternallyOwnedAccount {
  ...
  async createSmartContract(name, symbol) {
    try {
      ...
      throw new Error(
        '[ExternallyOwnedAccount.createSmartContract] error in creation (insufficient funds? noNetwork?)!',
      );
    }
  }
   ...
   * @param  {object} provider
   * @returns {object} Nft with metadata
   */
  async mintNft({ contractAddress, publicAddress, NFTUrl }) {
    ...
    return contract.mintWithTokenURI(publicAddress, NFTUrl);
  }
  /**
