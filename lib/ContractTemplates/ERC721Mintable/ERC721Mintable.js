...
export default class ERC721Mintable {
  ...
  async setApprovalForAll(address, approvalStatus) {
    ...
    if (!approvalStatus) {
      throw new Error(
        '[ERC721Mintable.setApprovalForAll] An approvalStatus is required to setApprovalForAll.',
      );
    }

    try {
      return this.#contractDeployed.setApprovalForAll(address, approvalStatus);
    } catch (error) {
