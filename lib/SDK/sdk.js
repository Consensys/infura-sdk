...
export default class SDK {
  ...
  async deploy({ template, params }) {
    ...
    if (Object.keys(params).length === 0) throw new Error('A set of parameters are required to deploy a new contract.');
  ...
  async getNFTs(address, includeMetadata = false) {
    ...
    const apiUrl = `${this.#apiPath}/accounts/${address}/assets/nfts`;
  ...
  async getEthBalance(address) {
    ...
    const apiUrl = `${this.#apiPath}/accounts/${address}/assets/eth`;
  ...
  async getERC20Balances(address) {
    ...
    const apiUrl = `${this.#apiPath}/accounts/${address}/assets/erc20`;
