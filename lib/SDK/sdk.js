...
export default class SDK {
  ...
  /**
   * Set royalty by address
   * @param {string, number} address, fee
   * @returns {boolean} Operation result
   */
  async setRoyalty(address, fee) {
    if (!address) {
      throw new Error('[SDK.setRoyalty] Address is required.');
    }

    if (!ethers.utils.isAddress(address)) {
      throw new Error('[SDK.setRoyalty] Address is not valid.');
    }

    if (!fee) {
      throw new Error('[SDK.setRoyalty] Fee is required.');
    }

    if (!Number.isInteger(fee)) {
      throw new Error('[SDK.setRoyalty] Fee needs to be a numeric value as a whole number.');
    }

    if (!(fee > 0) && !(fee < 10000)) {
      throw new Error('[SDK.setRoyalty] Fee needs to be a numeric value between 0 and 10000');
    }

    return this.contract.setRoyalties(address, fee);
  }
}
