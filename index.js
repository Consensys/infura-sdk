...
const auth = new Auth({
  infuraSecret: '',
  infuraId: '',
  private_key: process.env.SUPER_PRIVATE,
  rpcUrl: '',
});

// class Auth() {
// determine whether to use infura secret and id and construct the URL
// or use rpcUrl
// validation of key
// }
...
// );
account.createSmartContract();
contract.deploy();
contract.name();

// READ
const api = new Api({ infuraDetails });
const name = api.getContractMetadata(contract.address())['name'];
contract.name();

// WRITE
contract = new BaseContract.deploy(TEMPLATES.NFTContractCollection);
contract.mint();
